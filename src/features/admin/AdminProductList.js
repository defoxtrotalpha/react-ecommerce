...
export default function AdminProductList() {
  ...
  const handleFilter = (e, filterType, option) => {
    ...
    if (e.target.checked) {
      newfilterArr.push({
        [filterType.id]: option.value,
      });
    } else {
      const index = newfilterArr.indexOf([filterType.id]);
  ...
  const handleSort = (e, option) => {
    const newsortArr = [{ _sort: option.sort }, { _order: option.order }];
  ...
  const handleDelete = (prodId) => {
    dispatch(deleteProductAsync(prodId));
  };
  ...
  return (
    <>
      {editProd && (
        <Navigate to="/admin/product-form" replace={true}></Navigate>
      )}
...
function ProductGrid({ products, handleDelete, handleEdit }) {
  return (
